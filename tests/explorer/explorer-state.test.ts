...
describe('ExplorerState 选择', () => {
  it('single 清除其他选中并设锚点', () => {
    const s = new ExplorerState('t1', 'saved:1');
...
describe('ExplorerState 历史', () => {
  it('pushCurrent / stepBack / stepForward', () => {
    const s = new ExplorerState('t1', 'saved:1'); // currentPath = '/'
    s.pushCurrent('/home');
...
describe('ExplorerState 连接身份', () => {
  it('保留字符串 connection key', () => {
    const s = new ExplorerState('t1', 'direct:abc');
    expect(s.connectionKey).toBe('direct:abc');
  });
});
